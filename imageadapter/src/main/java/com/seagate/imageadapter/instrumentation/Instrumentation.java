...
import com.seagate.imageadapter.logging.FLog;
...
public class Instrumentation {
    ...
    public void onSuccess(int size) {
        ...
        mPerfListener.reportSuccess(elapsedTime, size);
...
//        canvas.drawText("[" + mTag + "]", 10, 25, mPaint);
        ...
        long dt = mFinishTime - mStartTime;
        ...
        switch (mState) {
            case STARTED:
                ...
                break;
            case SUCCESS:
//                message = String.format("Loaded %d pix in %dms at %d pix/sec", mSize, dt, dt != 0 ? (mSize * 1000) / dt : -1);
                message = String.format("Loaded in %d ms",  dt);
                break;
            case FAILURE:
        ...
        canvas.drawText(message, 10, 20, mPaint);
    }
}
